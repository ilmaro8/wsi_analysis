...
import sys
import utils
from model import MoCo, copy_params, momentum_update
from data_augmentation import get_pipeline_augment
from dataloader import Dataset_instance
import torch.optim as optim
from loss_functions import ContrastiveLoss, NT_Xent
...
encoder = MoCo(CNN_TO_USE, in_dim=input_dim, out_dim=output_dim, intermediate_dim=hidden_dim, T=0.07)
momentum_encoder = MoCo(CNN_TO_USE, in_dim=input_dim, out_dim=output_dim, intermediate_dim=hidden_dim, T=0.07)
...
momentum_encoder = copy_params(encoder, momentum_encoder)
...
optimizer = LARS(encoder.parameters(), lr,
...
while (epoch<EPOCHS and early_stop_cont<EARLY_STOP_NUM):
	...
	img_loss_1 = 0.0
	img_loss_2 = 0.0
	img_loss = img_loss_1 + img_loss_2
	...
	for i in range(iterations):
		...
		with torch.autocast(device_type='cuda', dtype=torch.float16):
			...
			k = momentum_encoder(X)
			#k1 = momentum_encoder(X1)
			#k2 = momentum_encoder(X2)

			loss1 = criterion(q1, k)
			loss2 = criterion(q2, k)
			...
			momentum_encoder = momentum_update(encoder, momentum_encoder, momentum)
			...
			if (best_loss>img_loss):
				...
				try:
					...
					try:
						...
						torch.save(encoder, model_weights_filename)
