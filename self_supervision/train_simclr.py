...
#models_path = '/PATH/TO/simCLR/MODEL'

models_path = args.OUTPUT_PATH
os.makedirs(models_path, exist_ok=True)
models_path = models_path + 'simCLR/'
os.makedirs(models_path, exist_ok=True)
models_path = models_path + CNN_TO_USE + '/'
os.makedirs(models_path, exist_ok=True)
model_weights_filename = models_path + 'simCLR.pt'
...
while (epoch<EPOCHS and early_stop_cont<EARLY_STOP_NUM):
	...
	for i in range(iterations):
		...
		with torch.autocast(device_type='cuda', dtype=torch.float16):
			...
			loss1 = criterion(q1, q)
			loss2 = criterion(q2, q)
			loss3 = criterion(q1, q2)
