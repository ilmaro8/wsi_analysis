...
from model_multiclass import CLAM_MB, Encoder, TransMIL, DSMIL, Additive_MIL, Attention_MIL
from model_transformer import Transformer
from enum_multi import ALG, PHASE, SELF_SUPERVISION, TISSUE_TYPE

from utils import get_features_and_split, get_generator_instances, get_splits, generate_list_instances, save_prediction, save_loss_function, save_hyperparameters, filter_features
...
parser.add_argument('-f', '--CSV_FOLDER', help='folder where csv including IDs and classes are stored',type=str, default='')
...
testing_set_bag = Dataset_bag_multiclass(test_dataset[:,0], test_dataset[:,1:])
...
with torch.no_grad():
	for i in range(iterations):
		...
		outputs_wsi_np_img = np.squeeze(outputs_wsi_np_img)
...
accuracy = accuracy_score(y_true, y_pred, checkpoint_path, phase, None, DATASET)
kappa = kappa_score(y_true, y_pred, checkpoint_path, phase, None, DATASET)
f1_score_macro, f1_score_micro, f1_score_weighted = f1_scores(y_true, y_pred, checkpoint_path, phase, None, DATASET)
precision_score_macro, precision_score_micro = precisions(y_true, y_pred, checkpoint_path, phase, None, DATASET)
recall_score_macro, recall_score_micro = recalls(y_true, y_pred, checkpoint_path, phase, None, DATASET)

print("accuracy " + str(accuracy)) 
print("kappa_score " + str(kappa)) 
